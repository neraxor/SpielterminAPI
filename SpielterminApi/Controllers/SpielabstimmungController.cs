...
namespace SpielterminApi.Controllers
{
    ...
    public class SpielabstimmungController : ControllerBase
    {
        ...
        {
            var abstimmungExists = _context.Spielabstimmungs.Any(x=> x.Id == request.SpielvorschlagId && x.SpielvorschlagId == request.SpielerId && x.SpielgruppeId == request.SpielgruppeId);
            if (abstimmungExists)
            {
            ...
            _context.Spielabstimmungs.Add(spielabstimmung);
